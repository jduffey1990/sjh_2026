...
import { useBoard, useCompatibility, useSnapshot, store } from '../lib/useStore'
  ...
  if (!hasBackend) {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-ink-800 bg-ink-900/50 p-6 text-center">
        <h1 className="font-bold text-slate-100">Packing board not connected</h1>
        <p className="mt-2 text-sm leading-relaxed text-slate-400">
          ...
          <code className="text-slate-300">.env.local</code>, then run the SQL in{' '}
          <code className="text-slate-300">supabase/</code>.
        </p>
    ...
    <div className="space-y-5">
      <header className="topo overflow-hidden rounded-2xl border border-ink-800 p-6">
        <div className="flex flex-wrap items-start gap-4">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm leading-relaxed text-slate-400">
              Group gear only. Claim what you're carrying; drop a count to zero
              and it moves to Not Required.
            </p>
          </div>
        ...
        <blockquote className="mt-5 border-l-2 border-aspen-500/50 pl-3 text-[12px] italic leading-relaxed text-slate-400">
          “Everyone in your group does not need all of these items but the group
          as a whole should have a good repair kit.”
      ...
      {adding && <AddItem onDone={() => setAdding(false)} meId={me?.id ?? null} />}
