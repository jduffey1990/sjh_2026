...
export default function ItemTile({
  ...
  const changedBy = item.last_changed_by ? riderById(item.last_changed_by) : null

  return (
    <div
      className={`rounded-xl border transition-colors ${STATE_STYLE[state]}`}
    >
      {/* --- header ---------------------------------------------------- */}
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3
              className={[
                ...
                state === 'not-required' ? 'text-slate-400' : 'text-slate-100',
              ].join(' ')}
            >
              {item.name}
            </h3>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px]">
            ...
            <span className="text-slate-600">{fmtOz(Number(item.weight_oz))}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 p-3 pt-2.5">
          {mine ? (
            <>
              ...
              >
                {mine.packed ? `✓ Packed (${mine.qty})` : `In my bag (${mine.qty})`}
              </button>
            </>
          ) : (
            <button
      ...
      {open && (
        <div className="space-y-3 border-t border-ink-800/70 p-3.5">
              ...
              {claims.map((c) => {
                ...
                return (
                  <li key={c.id} className="flex items-center gap-2 text-[12px]">
                    ...
                    <span className="tabular-nums text-slate-500">×{c.qty}</span>
                    {c.packed && <span className="text-sage-400">✓ packed</span>}
                    <button
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-ink-800/70 pt-2.5 text-[11px] text-slate-600">
            ...
            <span>bulk {'●'.repeat(item.bulk)}{'○'.repeat(4 - item.bulk)}</span>
            {changedBy && (
              <span>
